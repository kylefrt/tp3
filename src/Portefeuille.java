public class Portefeuille {
    ...
    public boolean transfertDevise(Portefeuille destination, double montantJetons) {
        if (this.monnaie.getNom() != destination.monnaie.getNom()) return false; // si c'est pas le même type
        if (this.montant           < montantJetons               ) return false; // si le montant est inferieur
        if (montantJetons < 0     || destination == null         ) return false;
    ...
    public boolean achatDevise(double montantEuros) {
        ...
        this.montant += montantEuros / this.getMonnaie().getValeurDeJeton();
